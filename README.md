# mkl05z32-metronome
A digital metronome project implemented on the **NXP FRDM-KL05Z** development board (Cortex-M0+). The device allows users to set a tempo (BPM) via a serial terminal and provides both visual (LED) and audiable (loudspeaker) feedback.

## Features
* **Precise Tempo Generation**: Range from 30 to 240 BPM
* **UART Communication**: Control the metronome using commands (ON, OFF, UP, DOWN) via any serial terminal (e.g., PuTTY)
...
* **PWM Audio**: Tone generation using the TPM
